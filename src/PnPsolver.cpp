...
namespace myslam
{
    ...
    void PnPsolver::SetRansacParameters(double probability, int minInliers, int maxIterations, int minSet, float epsilon, float th2)//重定位
    {
    ...
    cv::Mat PnPsolver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)//重定位
    {
        bNoMore = false;
        vbInliers.clear();
        nInliers=0;
    ...
    void PnPsolver::reset_correspondences(void)//iterate
    {
        number_of_correspondences = 0;
    }

    void PnPsolver::add_correspondence(double X, double Y, double Z, double u, double v)//iterate
    {
    ...
    void PnPsolver::choose_control_points(void)//compute_pose
    {
    ...
    void PnPsolver::compute_barycentric_coordinates(void)//compute_pose
    {
    ...
    void PnPsolver::fill_M(CvMat * M,
                           const int row, const double * as, const double u, const double v)//compute_pose
    {
    ...
    void PnPsolver::compute_ccs(const double * betas, const double * ut)//compute_R_t
    {
        for(int i = 0; i < 4; i++)
    ...
    void PnPsolver::compute_pcs(void)//compute_R_t
    {
        for(int i = 0; i < number_of_correspondences; i++) {
            double * a = alphas + 4 * i;
            double * pc = pcs + 3 * i;
    ...
    double PnPsolver::compute_pose(double R[3][3], double t[3])//iterate
    {
    ...
    double PnPsolver::reprojection_error(const double R[3][3], const double t[3])//compute_R_t
    {
        double sum2 = 0.0;
    ...
    void PnPsolver::estimate_R_and_t(double R[3][3], double t[3])//compute_R_t
    {
        double pc0[3], pw0[3];
    ...
    void PnPsolver::solve_for_sign(void)//compute_R_t
    {
        if (pcs[2] < 0.0) {
            for(int i = 0; i < 4; i++)
                for(int j = 0; j < 3; j++)
    ...
    double PnPsolver::compute_R_and_t(const double * ut, const double * betas,
                                      double R[3][3], double t[3])//compute_pose
    {
...
// betas_approx_3 = [B11 B12 B22 B13 B23                    ]
    ...
    void PnPsolver::compute_L_6x10(const double * ut, double * l_6x10)//compute_pose
    {
        const double * v[4];
    ...
    void PnPsolver::compute_rho(double * rho)//compute_pose
    {
    ...
    void PnPsolver::compute_A_and_b_gauss_newton(const double * l_6x10, const double * rho,
                                                 double betas[4], CvMat * A, CvMat * b)//gauss_newton
    {
        for(int i = 0; i < 6; i++) {
    ...
    void PnPsolver::qr_solve(CvMat * A, CvMat * b, CvMat * X)//gauss_newton
    {
        static int max_nr = 0;
        static double * A1, * A2;
